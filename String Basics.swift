...
Write in your own favorite quote where indicated, and be sure to include escaped quotation marks. 
When finished, print the value of `favoriteQuote`.
...
let favoriteQuote = "My favorite quote is \"Keep your face always toward the sunshine, and shadows will fall behind you\"."
print(favoriteQuote)
...
/*
Declare a name constant and assign it your name as a string literal. 
Then declare an age constant and give it your current age as an Int. 
...
respectively. Create a fullName constant that uses string concatenation to combine firstName and lastName.
Print the value of fullName.
*/

let firstName = "Deemh"
let lastName = "Albaqami"
...
Insert `fullName`, `previousBest` and `newBest` where indicated. 
...
Assign name your name as a string literal in all lowercase. 
Write an if-else statement that checks to see if nameInCaps and name are the same.
If they are, print "The two strings are equal," otherwise print "The two strings are not equal."
*/

let nameInCaps = "Deemh"
let name = "deemh"
if nameInCaps == name{
...
/*
Write a new if-else statement that also checks to see if nameInCaps and name are the same.
However, this time use the lowercased() method on each constant to compare the lowercase version of the strings.
If they are equal, print the following statement using string interpolations:
...
/*
Imagine you are looking through a list of names to find any that end in "Jr."
Write an if statement below that will check if `junior` has the suffix "Jr." If it does, 
...
"You decide to do part of this with the contains(_:) method.
Write an if statement below that will check if `textToSearchThrough` contains `textToSearchFor`.
If it does, print "I found it!" Be sure to make this functionality case insensitive.
*/
...
You think it might be fun to incorporate some friendly competition into your fitness tracking app.
Users will be able to compete with friends in small fitness challenges.
However, to do this users will need a password-protected account.
Write an if-else statement below that will check that the entered user name and password match the stored user name and password. While the password should be case sensitive, users should be able to log in even if their entered user name has the wrong capitalization. 
If the user name and password match, print "You are now logged in!" Otherwise, print "Please check your user name and password and try again."
*/
...
/*
Now that users can log in, they need to be able to search through a list of users to find their friends.
This might normally be done by having the user enter a name, and then looping through all user names to see if a user name contains the search term entered.
You'll learn about loops later, so for now you'll just work through one cycle of that.
Imagine you are searching for a friend whose user name is StepChallenger.
You enter "step" into a search bar and the app begins to search.
When the app comes to the user name "stepchallenger," it checks to see if "StepChallenger" contains "step."
Using `userName` and `searchName` below, write an if-else statement that checks to see if `userName` contains the search term.
